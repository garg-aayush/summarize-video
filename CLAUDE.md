...
The summarize step needs `llama-server`. The orchestrator spawns it automatically at step 6 (after freeing whisper/pyannote VRAM) and stops it on exit — pass `--llama-server-bin PATH` if the binary isn't on `$PATH`. Step 6 also makes a small extra extraction call against the same loaded model that distills the YouTube description (`<id>.description.txt`) into an `<id>.episode_context.md` block, which is then prepended to the main summary call to ground speaker names and named entities. Skip with `--no-episode-context`; see `docs/summarize.md` § "Episode context". `steps/summarize.py` can also be run standalone:
...
- llama-server with `--jinja`: Gemma 4's chat template enables reasoning by default. Every chat call shows `reasoning-budget: activated` in the server log, and the model can spend hundreds-to-thousands of hidden `<think>` tokens before emitting visible content. For short structured-output calls (e.g. episode-context extraction) this can consume the entire `max_tokens` budget and return empty content — disable per-request via `"chat_template_kwargs": {"enable_thinking": false}` in the JSON body. Do **not** disable it for the main summarize call: A/B'd in docs/experiments.md and reasoning-off translated Hindi quotes to English, dropped Devanagari-named resources, and ignored half of the prepended `<episode_context>` block. Deliberation is what makes Gemma honor strict prompt constraints (verbatim quotes, named entities, language preservation).
